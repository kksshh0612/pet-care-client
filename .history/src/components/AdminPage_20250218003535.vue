  ...
  <div class="min-h-screen bg-[#F8F9FD] pt-20">
    ...
    <div v-if="showCodeGroupModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 w-full max-w-lg">
        <h3 class="text-lg font-bold mb-6">{{ isEditing ? '코드 그룹 수정' : '코드 그룹 등록' }}</h3>
...
export default {
  ...
  methods: {
    ...
    openCodeGroupModal(group = null) {
      ...
      this.codeGroupForm = group 
        ? { ...group }
        : { id: null, code: '', groupName: '', description: '' }
      this.showCodeGroupModal = true
    },
    async handleSubmitCodeGroup() {
      try {
        if (this.isEditing) {
          await axios.patch('/api/v1/code/group', this.codeGroupForm)
          alert('코드 그룹이 수정되었습니다.')
        } else {
          await axios.post('/api/v1/code/group', this.codeGroupForm)
