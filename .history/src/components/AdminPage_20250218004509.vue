...
export default {
  ...
  async created() {
    console.log('AdminPage created 훅 실행')
    try {
      // axios 인스턴스 설정 확인
  ...
  methods: {
    async fetchWaitingList() {
      try {
        ...
        const response = await axios.get('/api/v1/pet-sitter/waiting-approval-list', config)
        console.log('펫시터 목록 조회 응답:', response.data)
        this.petSitterProfiles = response.data.petSitterProfiles
      } catch (error) {
        console.error('승인 대기 목록 조회 실패:', {
    ...
    async fetchCodeGroups() {
      try {
        console.log('코드 그룹 목록 조회 요청 시작')
        const response = await axios.get('/api/v1/code/group')
        console.log('코드 그룹 목록 조회 응답:', response.data)
        this.codeGroups = response.data.codeGroups
      } catch (error) {
        console.error('코드 그룹 목록 조회 실패:', error.response || error)
