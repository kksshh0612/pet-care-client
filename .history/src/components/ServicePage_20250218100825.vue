  ...
  computed: {
    ...
    filteredServices() {
      ...
      return this.services.filter(service => {
        const serviceDate = new Date(service.availableDay)
        const filterFrom = new Date(this.startDate)
        const filterTo = new Date(this.endDate)
        
        return serviceDate >= filterFrom && serviceDate <= filterTo
      })
    },
    sortedServices() {
    ...
    async fetchPetSitterServices() {
      try {
        ...
        this.services = response.data.petSitterWorkResponses.map(service => ({
          name: service.petSitterName,
          introduction: service.introduction,
          serviceTypes: service.serviceTypes.map(type => this.serviceTypeLabels[type]),
          petSizes: service.availableSizes.map(size => this.petSizeLabels[size]),
